...
pairs(~ HouseStyle + OverallQual + OpenPorchSF + YearBuilt + YearRemodAdd + SalePrice, data = train_clean)
pairs(~ RoofMatl + ExterQual + ExterCond + Foundation + Heating + SalePrice, data = train_clean)
pairs(~ HeatingQC + CentralAir + X1stFlrSF + X2ndFlrSF + SalePrice, data = train_clean)
pairs(~ FullBath + HalfBath + WoodDeckSF + PavedDrive + SalePrice, data = train_clean)
pairs(~ Fireplaces + TotRmsAbvGrd + GrLivArea+  SaleCondition + SalePrice, data = train_clean)
```
...
train_final <- cbind(select(train_to_process, -Utilities, -Functional, -BsmtFinType2, -BsmtFinSF2, -BsmtHalfBath, -SalePrice), train_clean)
```
...
test_clean <- select(test_non_null, -MSSubClass, -LowQualFinSF, -X3SsnPorch, -ScreenPorch, -PoolArea, -MiscVal, -MoSold, -YrSold)
...
test_final <- cbind(select(test_to_process, -Utilities, -Functional, -BsmtFinType2, -BsmtFinSF2, -BsmtHalfBath), test_clean)
```
